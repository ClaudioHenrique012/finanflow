...
import { signInWithEmail, signUpWithEmail, signInWithGoogle } from './lib/dataService'
...
export default function Auth() {
  ...
  const handleGoogle = async () => {
    ...
    catch (e) { setError(e.message); setGoogleLoad(false) }
  }
  ...
  const s = {
    ...
    title: {
      ...
      fontSize: 22, color: '#F0F1F5', letterSpacing: '-.4px',
    },
    subtitle: { fontSize: 13, color: '#858AA3', marginBottom: 28, textAlign: 'center' },
    label: { fontSize: 12, color: '#858AA3', display: 'block', marginBottom: 7, textTransform: 'uppercase', letterSpacing: '.5px' },
    input: {
      width: '100%', background: '#1F2434', color: '#F0F1F5',
    ...
    icon: { position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: '#858AA3', pointerEvents: 'none' },
    btnPrimary: {
      width: '100%', background: '#3B82F6', color: '#fff',
      border: 'none', borderRadius: 9, padding: '12px', fontSize: 15,
      fontWeight: 600, cursor: 'pointer', marginTop: 4, fontFamily: 'inherit',
      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
      transition: 'background .15s',
    },
    btnGoogle: {
      width: '100%', background: 'transparent', color: '#F0F1F5',
      border: '1px solid rgba(255,255,255,0.13)', borderRadius: 9, padding: '11px',
    ...
    divider: {
      ...
      margin: '18px 0', color: '#858AA3', fontSize: 12,
    },
    ...
    toggle: { textAlign: 'center', marginTop: 22, fontSize: 13, color: '#858AA3' },
    toggleBtn: { color: '#3B82F6', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13 },
  }
  ...
  return (
    <>
      ...
      <div style={s.page}>
        <div style={s.card}>
          <div style={s.logo}>
            <div style={s.logoIcon}><Wallet size={20} color="#fff" /></div>
            <span style={s.title}>FinFlow</span>
          </div>

          <p style={s.subtitle}>
            {mode === 'login' ? 'Bem-vindo de volta! Entre na sua conta.' : 'Crie sua conta gratuita.'}
          </p>
          ...
          {/* Google */}
          <button style={s.btnGoogle} onClick={handleGoogle} disabled={googleLoad}>
            {googleLoad
              ...
              : (
                <svg width="17" height="17" viewBox="0 0 48 48">
                  ...
                  <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.6 16 19 13 24 13c3 0 5.7 1.1 7.8 2.9l5.7-5.7C33.8 6.5 29.2 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"/>
                  <path fill="#4CAF50" d="M24 44c5.2 0 9.9-1.9 13.5-5.1l-6.2-5.2C29.3 35.3 26.8 36 24 36c-5.2 0-9.6-3-11.3-7.2l-6.6 5.1C9.8 39.8 16.4 44 24 44z"/>
                  <path fill="#1976D2" d="M43.6 20H24v8h11.3c-.9 2.4-2.5 4.4-4.7 5.8l6.2 5.2C40.5 35.7 44 30.3 44 24c0-1.3-.1-2.7-.4-4z"/>
                </svg>
              )
            }
          ...
          <p style={s.toggle}>
            ...
            <button style={s.toggleBtn} onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); setSuccess('') }}>
